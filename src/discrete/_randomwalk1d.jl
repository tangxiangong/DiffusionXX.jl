...
输出函数
"""
Base.show(io::IO, ::MIME"text/plain", model::RandomWalk1D) = print(io, "一维随机游走, 模拟步数 $(model.step_num), 起点 $(model.start_point), 步长 $(model.step_size), 左/右转概率 $(model.prob)/$(1 - model.prob).")

"""
路径模拟
"""
function simulate(model::RandomWalk1D)
    # noise = [rand() < model.prob ? -1 : 1 for _ in 1:model.step_num]
